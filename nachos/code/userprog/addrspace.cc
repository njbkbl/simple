...
// then, copy in the code and data segments into memory
    ...
    #ifdef CHANGED
    nbThreadsMutex = new Semaphore("nb threads mutex", 1);
    #endif
}
...
int
AddrSpace::getNbThreadsRunning (){
  return nbThreadsRunning;
}

void
AddrSpace::addNbThreadRunning () {
  nbThreadsMutex->P();
  nbThreadsRunning++;
  nbThreadsMutex->V();
}
...
AddrSpace::AllocateUserStack(){
  int stackPtr;
  nbThreadsMutex->P();
  stackPtr = numPages * PageSize - 16 - 256*nbThreadsRunning;
  nbThreadsMutex->V();
  return stackPtr;
}
